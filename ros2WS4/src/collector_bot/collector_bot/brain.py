...
from collector_bot.obstacle_memory import ObstacleMemory
...
class CollectorBrain(Node):
    ...
    def _do_explore(self):
        ...
        # Check obstacle memory for detour
        detour = self.obs_mem.get_detour(self.x, self.y, target[0], target[1])
        nav_target = detour if detour else target
        nav_vx, nav_vy, nav_wz, _ = go_to_point(
            nav_target[0], nav_target[1], self.x, self.y, self.yaw, arrival_dist=0.8)
    ...
    def _do_deliver(self):
        ...
        # Light repulsion — basket is near walls, must push through
        rep_vx, rep_vy = self._repulsion()
        vx = nav_vx + rep_vx * 0.25
        vy = nav_vy + rep_vy * 0.25
        self._pub(vx, vy, nav_wz)
    ...
    def _do_drop(self):
        ...
        if self._drop_future.done():
            ...
            self._broadcast('/dropped', self.carrying_name)
            self.carrying = None
            self.carrying_name = None
            self._drop_future = None
            self.state = self.EXPLORE
    ...
    def _check_stuck(self, now):
        ...
        if math.sqrt(dx * dx + dy * dy) < STUCK_DIST:
            ...
            self._esc_t0 = now
            self._pos_hist.clear()
            # Mark obstacle in memory
            heading = self.yaw
            if self._esc_target:
                heading = math.atan2(
                    self._esc_target[1] - self.y,
                    self._esc_target[0] - self.x)
            self.obs_mem.mark_stuck(self.x, self.y, heading, self.scan_ranges)
            if self.state == self.APPROACH:
                self._release_claim()
            self.state = self.ESCAPE
            self.get_logger().warn(f'[{self.ns}] STUCK → escape (marked obstacle)')
