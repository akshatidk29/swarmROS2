...
Industrial / utilitarian aesthetic -- no gradients, strict grid, monochrome + cyan accent.
"""
...
PANEL     = "#141414"   # slightly lighter panel
GRID_COL  = "#222222"   # subtle grid lines
BORDER    = "#333333"   # border / rule colour
TEXT_PRI  = "#E8E8E8"   # primary text (off-white)
TEXT_SEC  = "#888888"   # secondary / dim text
ACCENT    = "#00C8D4"   # robotic cyan accent (single pop colour)
DANGER    = "#CC3333"   # collision / alert
SAFE      = "#2A7A5A"   # safe / ok
BLUE_TRK  = "#2255AA"   # trajectory blue (darker, distinct from cyan accent)
...
def draw_world(ax):
    ...
    # Shelves - Defined by (cx, cy, width, height, rotation_deg)
    shelves = [
        (-2.5,  2.0, 1.4, 0.4, 0),   # shelf_1a (Pitched 45 - projected footprint ~1.4)
        (-2.5,  0.0, 2.0, 0.4, 0),   # shelf_1b
        (-2.5, -2.0, 2.0, 0.4, 0),   # shelf_1c
        ( 0.5,  2.0, 2.0, 0.4, 0),   # shelf_2a
        ( 0.5,  0.0, 2.0, 0.4, 0),   # shelf_2b
        ( 0.5, -2.0, 2.0, 0.4, 0),   # shelf_2c
        ( 3.5,  2.0, 1.5, 0.4, 20),  # shelf_3a (Rotated 20 deg)
        ( 3.5, -2.0, 1.5, 0.4, 0),   # shelf_3b
    ]
    for cx, cy, w, h, angle in shelves:
        # Calculate rotated rectangle using a transform to ensure rotation is around center
    ...
    # Bins — single-colour fill, no hatch (clean)
    bins_data = [
